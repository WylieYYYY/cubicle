...
fn main() {
    ...
    for entry in fs::read_dir(build_dir.join(context_file).parent().unwrap()).unwrap() {
        ...
        if let Some(view_name) =
            entry_filename.strip_suffix(&(String::from(".") + &context_extension))
        {
            if view_name == context_filename {
                continue;
            }
    ...
    tera.add_template_file(resource_dir.join(context_file), Some(&context_filename))
        .unwrap();
    context.insert("view_names", &view_names);
    let context_file = &File::create(build_dir.join(context_file)).unwrap();
    tera.render_to(&context_filename, &context, context_file)
        .unwrap();
}
